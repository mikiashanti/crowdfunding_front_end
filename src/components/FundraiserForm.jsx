...
import "./FundraiserForm.css"; // import the CSS
...
export default function FundraiserForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = { title, description, goal: Number(goal), image, is_open: true };
      ...
      if (res.ok) {
        ...
        const msg = errBody?.detail ?? errBody?.message ?? "Failed to create fundraiser";
        alert(msg);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <form className="fundraiser-form" onSubmit={handleSubmit}>
      ...
      <input type="number" value={goal} onChange={(e) => setGoal(e.target.value)} placeholder="Goal (USD)" required />
